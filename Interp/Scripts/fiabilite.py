...
# Dans le manuel, l'interpolation de Chebyshev n'est pas 
# présentée
avec_chebyshev = True

# Dans le manuel, l'interpolation par Spline n'est pas 
# présentée
avec_spline = True

# 1. Calcule les données du problème

# Les données sont fournies avec la précision absolue suivante
precision_absolue_donnees = 1.0e-3
...
pl.plot(beta_array, precision_absolue_donnees * np.ones(number_of_points))
pl.plot(beta_data, np.zeros(number_of_data_points), "|")
pl.xlabel(u"$\\beta$")
...
pl.plot(beta_array, erreur_global, "--", label="Équidistant")
if avec_chebyshev:
...
nom_fichier = "fiabilite-interpolation"
if avec_chebyshev:
    nom_fichier += "_Chebyshev"
if avec_spline:
    nom_fichier += "_Spline"
nom_fichier += ".pdf"
print("Ecrit : ", nom_fichier)
pl.savefig(nom_fichier, bbox_inches="tight")
...
pl.plot(number_of_points_array, error_equi, "--", label="Équidistant")
if avec_chebyshev:
...
pl.legend(bbox_to_anchor=(1.0, 1.0))
nom_fichier = "fiabilite-convergence"
if avec_chebyshev:
    nom_fichier += "_Chebyshev"
if avec_spline:
    nom_fichier += "_Spline"
